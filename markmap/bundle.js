(function (prosemirrorSchemaBasic, prosemirrorSchemaList, prosemirrorState, prosemirrorView, prosemirrorKeymap, prosemirrorHistory, prosemirrorCommands, prosemirrorMarkdown, markmapLib, markmapView) {
  'use strict';

  const mySchema = prosemirrorSchemaList.addListNodes(prosemirrorSchemaBasic.schema, 'paragraph block*', 'block');

  let view = null;
  let currentMarkmap = null;

  function getMarkdown() {
    return prosemirrorMarkdown.defaultMarkdownSerializer.serialize(view.state.doc);
  }
  ...
  function updateMarkmap() {
    ...
    try {
      ...
      currentMarkmap = markmapView.Markmap.create(svg, null, root);
  ...
  const state = prosemirrorState.EditorState.create({
    ...
    plugins: [prosemirrorHistory.history(), prosemirrorKeymap.keymap(prosemirrorCommands.baseKeymap)]
  });

  view = new prosemirrorView.EditorView(document.getElementById('editor'), {
    state,
...
})(prosemirrorSchemaBasic, prosemirrorSchemaList, prosemirrorState, prosemirrorView, prosemirrorKeymap, prosemirrorHistory, prosemirrorCommands, prosemirrorMarkdown, markmapLib, markmapView);
